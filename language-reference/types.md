...
불투명한 타입은 함수나 서브스크립트의 반환 타입,
함수, 서브스크립트, 초기화 구문의 파라미터 타입,
또는 프로퍼티의 타입으로 사용할 수 있습니다.
불투명한 타입은 튜플 타입의 일부로 사용할 수 없으며,
배열의 요소 타입이나 옵셔널의 감싸진 타입처럼 제너릭 타입의 일부로도 사용할 수 없습니다.
...
파라미터에 불투명한 타입을 사용하는 것은
제너릭 타입을 사용하는 것과 동일하지만,
제너릭 타입 파라미터의 이름을 명시하지 않는 문법적 단축입니다.
암시적으로 생성된 제너릭 타입 파라미터는
불투명한 타입에서 지정한 프로토콜을 준수해야 한다는 제약을 가집니다.
여러 개의 불투명한 타입을 사용하면,
각각 독립적인 제너릭 타입 파라미터가 생성됩니다.
예를 들어 다음 선언들은 동일한 의미를 가집니다:
...
두 번째 선언에서는
제너릭 타입 파라미터 `T1`과 `T2`가 이름을 가지므로,
코드의 다른 곳에서 이 타입들을 참조할 수 있습니다.
반면에,
첫 번째 선언의 제너릭 타입 파라미터는
이름이 없으며 다른 코드에서 참조할 수 없습니다.

불투명한 타입은 가변 파라미터의 타입으로 사용할 수 없습니다.

불투명한 타입을
반환하는 함수 타입의 파라미터로 사용하거나,
함수 타입을 파라미터로 가지는 타입의 파라미터로 사용할 수도 없습니다.
이러한 경우,
함수의 호출자가 알 수 없는 타입의 값을
생성해야 하기 때문입니다.
