...
일반적인 형식에서 가용성 조건은 플랫폼 이름 및 버전 리스트를 사용합니다. `iOS`, `macOS`, `watchOS`, `tvOS`, 그리고 `visionOS` 와 같은 플랫폼 이름을 사용하고 자세한 내용은 [선언 속성 \(Declaration Attributes\)](../language-reference/attributes.md#선언-속성-declaration-attributes) 을 참고 바랍니다. iOS 8 또는 macOS 10.10과 같은 메이저 버전 번호를 지정하는 것 외에 iOS 11.2.6과 macOS 10.13.3과 같은 마이너 버전 번호로 지정할 수 있습니다.
